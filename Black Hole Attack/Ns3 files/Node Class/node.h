 ...
 * 
 * -----------------------------------------------------------------
 * Edições no código originais realizadas por Agnaldo Batista - UFPR 
 * e João Monteiro Delveaux Silva - UFMG <joaomont@ufmg.br>
 *------------------------------------------------------------------
 *
 */
#ifndef NODE_H
...
*/
  ...
  void RegisterMaliciousNeighbor (Ipv4Address ip, bool state, int recurrence, double timestamp);
  void UnregisterMaliciousNeighbor (Ipv4Address ip);

  bool GetMaliciousNeighborState (Ipv4Address ip);
  void SetMaliciousNeighborState (Ipv4Address ip, bool state);
  double GetMaliciousNeighborRecurrence (Ipv4Address ip);
  void IncreaseMaliciousNeighborRecurrence (Ipv4Address ip);
  void IncreaseMaliciousNeighborMsgSent (Ipv4Address ip);
  void IncreaseMaliciousNeighborMsgReceived (Ipv4Address ip);
  int GetMaliciousNeighborMsgReceived (Ipv4Address ip);
  int GetMaliciousNeighborMsgSent (Ipv4Address ip);
  void DecreaseMaliciousNeighborRecurrence (Ipv4Address ip);
  double GetMaliciousNeighborTimestamp (Ipv4Address ip);
  void SetMaliciousNeighborTimestamp (Ipv4Address ip, double time);
  double GetMaliciousNeighborTimeToBlock (Ipv4Address ip);
  void SetMaliciousNeighborTimeToBlock (Ipv4Address ip, double time);

  bool IsThereAnyMaliciousNeighbor ();
  std::vector<Ipv4Address> GetMaliciousNeighborIpList ();
  bool IsAlreadyMaliciousNeighbor (Ipv4Address ip);
  int GetNMaliciousNeighbors ();
  void ClearMaliciousNeighborList ();

  void addNeighborSender (Ipv4Address neighbor, Ipv4Address ip);
  bool IsAlreadyNeighborSender (Ipv4Address neighbor, Ipv4Address ip);
  void removeNeighborSender (Ipv4Address neighbor, Ipv4Address ip);
  void printNeighborSenders(Ipv4Address neighbor,std::ostream &log);
  void printNeighborsSendersList(std::ostream &log);
  std::list<Ipv4Address> getNeighborSenders (Ipv4Address ip);
