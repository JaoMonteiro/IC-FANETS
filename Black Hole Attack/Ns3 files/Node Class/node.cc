...
void
Node::RegisterNeighbor (Ipv4Address ip, Vector position, double distance, uint8_t attitude, uint8_t quality, uint8_t hop, double time)
{
  NS_LOG_FUNCTION (this);
 ...
 */
void
Node::UpdateNeighbor (Ipv4Address ip, Vector position, double distance, uint8_t attitude, uint8_t quality, uint8_t hop, double time)
{
  NS_LOG_FUNCTION (this);
  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end ();
        i++)
...
/**
 * @brief Get the time information of a neighbor node
 * @date Mar 5, 2023
 * 
 * @param ip - Neighbor node IPv4 address
...
double
Node::GetMaliciousNeighborTimestamp (Ipv4Address ip)
{
  double time;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
      if (i->ip == ip) 
      {
        time = i->timestamp; 
...
void
Node::RegisterMaliciousNeighbor (Ipv4Address ip, bool state, int recurrence, double timestamp )
{
  NS_LOG_FUNCTION (this);
  struct Node::MaliciousNeighbor neighbor;

  neighbor.ip = ip;
  neighbor.state = state;
  neighbor.recurrence = recurrence;
  neighbor.timestamp = timestamp;
  neighbor.msgReceived = 0;
  neighbor.msgSent = 0;
  neighbor.timeToBlock = 0;
  m_maliciousNeighborList.push_back (neighbor);
}

std::vector<Ipv4Address>
Node::GetMaliciousNeighborIpList ()
{
  NS_LOG_FUNCTION (this);
  std::vector<Ipv4Address> NeighborIpList;
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
void
Node::UnregisterMaliciousNeighbor (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  if (IsAlreadyMaliciousNeighbor (ip)) // checks if is already a neighbor
    {
      for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end ();
           i++)
        {
          if (i->ip == ip)
            {
              m_maliciousNeighborList.erase (i);
              break;
            }
        }
    }
...
double
Node::GetMaliciousNeighborRecurrence (Ipv4Address ip)
{
  int recurrence;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
double
Node::GetMaliciousNeighborTimeToBlock (Ipv4Address ip)
{
  double timeToBlock;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
void
Node::IncreaseMaliciousNeighborRecurrence (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
void
Node::IncreaseMaliciousNeighborMsgSent (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
void
Node::IncreaseMaliciousNeighborMsgReceived (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
void
Node::DecreaseMaliciousNeighborRecurrence (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
bool
Node::GetMaliciousNeighborState (Ipv4Address ip)
{
  bool state;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
int
Node::GetMaliciousNeighborMsgSent (Ipv4Address ip)
{
  int msg;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
int
Node::GetMaliciousNeighborMsgReceived (Ipv4Address ip)
{
  int msg;
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
...
void
Node::SetMaliciousNeighborState (Ipv4Address ip, bool state)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
void
Node::SetMaliciousNeighborTimestamp (Ipv4Address ip, double time)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
void
Node::SetMaliciousNeighborTimeToBlock (Ipv4Address ip, double time)
{
  NS_LOG_FUNCTION (this);
  for (MaliciousNeighborHandlerList::iterator i = m_maliciousNeighborList.begin (); i != m_maliciousNeighborList.end (); i++)
    {
...
bool
Node::IsThereAnyMaliciousNeighbor ()
{
  NS_LOG_FUNCTION (this);
  return !m_maliciousNeighborList.empty ();
}

bool
Node::IsAlreadyMaliciousNeighbor (Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  bool inTheList = false;
...
int
Node::GetNMaliciousNeighbors (void)
{
  NS_LOG_FUNCTION (this);
  return (int) m_maliciousNeighborList.size ();
}

void
Node::ClearMaliciousNeighborList ()
{
  NS_LOG_FUNCTION (this);
  m_maliciousNeighborList.clear();
}

void
Node::addNeighborSender (Ipv4Address neighbor, Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    if (i->ip == neighbor) {
      i->possibleNeighbors.push_back(ip); 
      break;
    }
  }
}
...
bool
Node::IsAlreadyNeighborSender (Ipv4Address neighbor, Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  bool inTheList = false;
  ...
  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    if (i->ip == neighbor) {
      for (std::list<Ipv4Address>::iterator it = i->possibleNeighbors.begin (); it != i->possibleNeighbors.end(); it++) {
        if(*(it) == ip)
          {inTheList=true;break;}
      }
      break;
    }
  }
...
void
Node::removeNeighborSender (Ipv4Address neighbor, Ipv4Address ip)
{
  NS_LOG_FUNCTION (this);
  ...
  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    if (i->ip == neighbor) {
      for (std::list<Ipv4Address>::iterator it = i->possibleNeighbors.begin (); it != i->possibleNeighbors.end(); it++) {
        if(*(it) == ip)
          {i->possibleNeighbors.erase(it);break;}
      }
      break;
    }
  }
...
void Node::printNeighborSenders(Ipv4Address neighbor, std::ostream &log = std::cout) {

  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    if (i->ip == neighbor && i->possibleNeighbors.size() > 0) {
      
      // Print first outside of loop because of commas
      std::list<Ipv4Address>::iterator it = i->possibleNeighbors.begin ();
      log << *(it);
      it++;
      
      for (; it != i->possibleNeighbors.end(); it++) 
        { log << ',' << *(it); }
      break;
    }
  }
...
void Node::printNeighborsSendersList(std::ostream &log = std::cout) {

  std::cout << "Neighbors Senders list:\n";

  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    log << i->ip << ": ";
    printNeighborSenders(i->ip,log);
    log << std::endl;
  } 

}

std::list<Ipv4Address> Node::getNeighborSenders (Ipv4Address ip) {

  std::list<Ipv4Address> neighbors;

  for (NeighborHandlerList::iterator i = m_neighborList.begin (); i != m_neighborList.end (); i++) {
    if (i->ip == ip) 
      { neighbors = i->possibleNeighbors;break; }
    }
