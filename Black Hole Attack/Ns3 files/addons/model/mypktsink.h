...
public:
  ...
  // Custom Functions and attribute by Joao Monteiro D. Silva
    ...
    bool m_malicious;       //!< Indicates if aplication in sending malicious information
    unsigned m_seed;
...
private:
  ...
  // Custom Callbacks by Joao Monteiro D. Silva
    ...
    TracedCallback<const Ipv4Address &, const Ipv4Address &, const Ipv4Address &, MyTag &, const double &, const Ptr <ns3::Node>& > m_myMaliciousRxTrace;
    
    TracedCallback<const Ipv4Address &, const Ipv4Address &, MyTag &, const double &, const Ptr <Node>&> m_myMaliciousTxTrace;

    TracedCallback<const Ptr <Node>&, const Ipv4Address &,const double &,const double &, const int &, const bool &> m_mySuspiciousTrace;
  ...
  // Custom Functions by Joao Monteiro D. Silva
    ...
    double calculateDistanceBetweenTwoPoints(Vector a,Vector b);
    ...
    void printMaliciousNeighborList();
    
    void notifyAboutNeighbor (Ipv4Address ip, uint8_t tagType);

    void checkIfMaliciousNeighbor(Ipv4Address ip);

    std::vector<ns3::Ipv4Address> buildSlidingWindow (Ipv4Address suspectIp);
};
