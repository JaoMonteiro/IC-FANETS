...
*/
  ...
  while(it  != neighborhoodIPList.end()) {
    ...
    if(appNode->GetNeighborQuality(*it) == 0 ) {
      ...
      //std::cout << timeNow << '\t' << m_nodeIP << " removed: " << *it << std::endl;
      //printNeighbor(*it);

      if(!appNode->IsAlreadyMaliciousNeighbor(*it))
        {appNode->UnregisterNeighbor(*it);}
      else 
        {appNode->SetNeighborQuality(*it,1);}
      
      if(it != neighborhoodIPList.end())
        {it++;}
        
    } else if((appNode->GetNeighborHop(*it) > 1 ) && ( appNode->GetNeighborDistance(*it) - appNode->GetNeighborAttitude(*it) < 87)) {
  
      // Check if there are neighbors incorrectly identified as hop 2 or greater
  ...
  neighborhoodIPList = appNode->GetMaliciousNeighborIpList();

  for(unsigned int i = 0; i < neighborhoodIPList.size(); i++ ) {
    if(appNode->IsAlreadyNeighbor(neighborhoodIPList.at(i)))
      appNode->SetNeighborState(neighborhoodIPList.at(i),true);
  }
  ...
  // Send packets based on the size of the neighborhood and node mobility
    ...
    if( !appNode->IsThereAnyNeighbor() || !m_oprStarted || noCloseNeighbors ) { 
      ...
      for(unsigned int i = 0; i < neighborhoodIPList.size(); i++) {
        ...
        if(appNode->GetNeighborHop(neighborhoodIPList.at(i)) == 1 && !appNode->IsAlreadyMaliciousNeighbor(neighborhoodIPList.at(i))) {            
          
          // Send message only to close, trustworthy neighbors

          if(appNode->IsAlreadyMaliciousNeighbor(neighborhoodIPList.at(i)))
            {appNode->IncreaseMaliciousNeighborMsgSent(neighborhoodIPList.at(i));}
