...
void Statistics::senderCallback(const Ipv4Address &from , const Ipv4Address &to, MyTag &tag, const double &time, const Ptr <Node>& m_node) {
  ...
  case 3:
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          ...
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor (Ip, Recurrance)" << std::endl;}
    ...
    if(!logFile.is_open())
        ...
        { 
          ...
          vector<MyTag::NeighInfos> neighbourhood = tag.GetNeighInfosVector();
          for(unsigned int i = 0 ; i < neighbourhood.size(); i++) {
            logFile << neighbourhood.at(i).ip << ", " << (int) neighbourhood.at(i).hop << '\t';
          }
          logFile << std::endl;
          logFile.close();
        }
  ...
  case 4:
      ...
      //-------------------------------------
      // The message sent is an update of a blocked neighbor
      //-------------------------------------
    ...
    if(!fileExists(filename.str())) {
      ...
      if(!logFile.is_open())
          ...
          { logFile << "Time\tDestination\tPosition\tTagType\tNeighbor" << std::endl;}
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          ...
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor" << std::endl;}
    ...
    if(!logFile.is_open())
        ...
        { 
          logFile << time << '\t' << to << '\t' << (int) tag.GetSimpleValue() << '\t' << '1' << '\t';
  ...
  case 5:
      ...
      //-------------------------------------
      // The message sent is an unregistration of a suspicious neighbor
      //-------------------------------------
    ...
    if(!fileExists(filename.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.senderCallback - Removed Suspicious Neighbor" << std::endl;}
        else 
          { logFile << "Time\tDestination\tPosition\tTagType\tNeighbor" << std::endl;}
  ...
  case 6:
      ...
      //-------------------------------------
      // The message sent is a suspicious decrease in a neighbor
      //-------------------------------------
    ...
    if(!fileExists(filename.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.senderCallback - Decreased Suspicious Neighbor" << std::endl;}
        else 
          { logFile << "Time\tDestination\tPosition\tTagType\tNeighbor" << std::endl;}
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.senderCallback - Decreased Suspicious" << std::endl;}
        else 
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor" << std::endl;}
...
void Statistics::receiverCallback(const Ipv4Address &m_nodeIP , const Ipv4Address &from , const Ipv4Address &to, MyTag &tag, const double &time, const Ptr <Node>& m_node) {
...
case 0:
  ...
  case 4:
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.receivercallback - Blocked" << std::endl;}
        else 
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor" << std::endl;}
    ...
    if(!logFile.is_open())
        ...
        { 
          logFile << time << '\t' << from << '\t' << (int) tag.GetSimpleValue() << '\t' << '1' << '\t';
  ...
  case 5:
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.receivercallback - Removed Suspicious" << std::endl;}
        else 
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor" << std::endl;}
    ...
    if(!logFile.is_open())
        ...
        { 
          logFile << time << '\t' << from << '\t' << (int) tag.GetSimpleValue() << '\t' << '0' << '\t';
  ...
  case 6:
    ...
    if(!fileExists(filename.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.receivercallback - Decreased Suspicious Neighbor" << std::endl;}
        else 
          { logFile << "Time\tLatency\tFrom\tPosition\tTagType\tNeighborhoodSize\tNeighbors (Ip, Distance, Hop)" << std::endl;}
    ...
    if(!fileExists(filename2.str())) {
      ...
      if(!logFile.is_open())
          {std::cout << "Error trying to open file in statistics.receivercallback - Decreased Suspicious" << std::endl;}
        else 
          { logFile << "Time\tDestination\tTagType\tBlockedState\tNeighbor" << std::endl;}
...
void Statistics::printTotalTraces() {
  ...
  std::cout << "\nTotal Traces:\n\n"
            ...
            << "\nBlock Messages sent: " << m_tag4Sent << "\nBlock Messages received: " << m_tag4Received << std::endl
            << "\nRemoved from SL Messages sent: " << m_tag5Sent << "\nRemoved from SL Messages received: " << m_tag5Received << std::endl
            << "\nDecreased Suspicious Messages sent: " << m_tag6Sent << "\nDecreased Suspicious Messages received: " << m_tag6Received << std::endl
      ...
      if(ipList.size() > 0) {
        ...
        for( unsigned int i = 0; i < simulatorIPv4.size(); i++) {
          ...
          for(unsigned int u = 0; u < ipList.size(); u++) {
            ...
            if(simulatorIPv4.at(i) == ipList.at(u) && (*it)->GetNeighborHop(ipList.at(u)) == 1 && (*it)->IsAlreadyMaliciousNeighbor(ipList.at(u))) {
   ...
   // Create the trace file header
    ...
    if(!fileExists(filename.str())) {
      ...
      if(!logFile.is_open())
          ...
          logFile << "Time\tPosition\t Neighbors (Ip, Position, Distance, Quality, InfoTime, Hop, State)";
          logFile << std::endl;
        }
      logFile.close();
    }
...
void Statistics::printNeighbor(Ipv4Address ip, Ptr<Node> m_node,std::ofstream &log) {
  ...
  << m_node->GetNeighborInfoTime(ip) << ", " << att << ", " << (int) m_node->GetNeighborHop(ip) << ", " << (int) m_node->GetNeighborState(ip) << '\t' ;
}
...
void Statistics::maliciousReceiverCallback(const Ipv4Address &m_nodeIP , const Ipv4Address &from , const Ipv4Address &to, MyTag &tag, const double &time, const Ptr <Node>&m_node) {
  ...
  receiverCallback(m_nodeIP,from,to,tag,time,m_node);
}
