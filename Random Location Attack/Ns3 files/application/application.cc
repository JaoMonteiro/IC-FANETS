...
/*
  ...
  BE CAREFUL! 
    ...
    - Maximum nodes is 40
    - Simulation runtime is defined at line 324
    - Depending of the number of malicious drones, they are deployed differently. Check lines 283 - 319
    - To use NetAnim, uncomment lines 252 - 262
    - To use NetSimulyzer, after installing it and its module, uncomment lines 41 - 51 and 264 - 281    
...
statistics.addSimulationInfo("Altitude of nodes is 91.4m\n");
  ...
  for (it = Nodes.Begin(); it != Nodes.End(); it++) {
    ...
    start += 0.1; // Avoid to start all the OnOff together
  }
  ...
  if(maliciousDrones != 3) {
    for( unsigned int i = 0; i < maliciousDrones;i++) {
      ...
      text << "Node " << maliciousNodes[i] << " will turn malicious in " << t.ToDouble(Time::S) << "s"<< std::endl;
    }
   } else {
